...
Ported from PR #36 onto the integrated add-mnl-model branch: this branch's
``mnl_ate`` / ``mnl_ate_from_params`` take ``X`` keyword-only (they also accept
``data``/``spec``/``alternatives`` and a ``scenarios=`` API), so ``X`` is
passed as a keyword here.
"""
...
def test_from_estimates_sets_nan_se_and_preserves_b(mnl_fit):
    ...
    assert r.converged is True and r.return_code == 0
